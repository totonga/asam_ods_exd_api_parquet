    ...
    def GetValues(self, request, context):
        ...
        for channel_id in request.channel_ids:
            ...
            if ods.DataTypeEnum.DT_BYTE == ods_data_type:
                ...
                new_channel_values.values.longlong_array.values[:] = np.array(channel[request.start:end_index], np.int64)
            elif ods.DataTypeEnum.DT_FLOAT == ods_data_type:
                new_channel_values.values.float_array.values[:] = np.array(channel[request.start:end_index], np.float32)
            elif ods.DataTypeEnum.DT_DOUBLE == ods_data_type:
                new_channel_values.values.double_array.values[:] = np.array(channel[request.start:end_index], np.float64)
                ...
                new_channel_values.values.string_array.values[:] = np.array(channel[request.start:end_index], np.string_)
            else:
