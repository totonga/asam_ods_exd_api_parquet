...
class TestFileAccess(unittest.TestCase):
    ...
    def test_create_example_all_datatypes(self):
        ...
        with tempfile.TemporaryDirectory() as temporary_directory_name:
            ...
            # ['int8_data', 'uint8_data', 'int16_data', 'uint16_data', 'int32_data', 'uint32_data', 'int64_data', 'uint64_data', 'date_data', 'float32_data', 'float64_data', 'string_data']
            # [DataType(int8), DataType(uint8), DataType(int16), DataType(uint16), DataType(int32), DataType(uint32), DataType(int64), DataType(uint64), TimestampType(timestamp[us]), DataType(float), DataType(double), DataType(string)]
