...
class TestDataTypes(unittest.TestCase):
    ...
    def test_datatype(self):
        with tempfile.TemporaryDirectory() as temporary_directory_name:
            ...
            df = pd.DataFrame(
                {
                    ...
                    "string_data": ["abc", "def"],
                    "date_ns_data": pd.to_datetime([datetime(2017, 7, 9, 12, 35, 0), datetime(2017, 7, 9, 12, 36, 0)]),
            ...
            handle = service.Open(
                exd_api.Identifier(url=pathlib.Path(file_path).resolve().as_uri(), parameters=""), self.context
            )
            try:
                ...
                self.assertEqual(structure.groups[0].channels[12].data_type, ods.DataTypeEnum.DT_DATE)
                self.assertEqual(structure.groups[0].channels[13].data_type, ods.DataTypeEnum.DT_DATE)
                ...
                self.assertSequenceEqual(
                    values.channels[8].values.string_array.values, ["20170709123500", "20170709123600"]
                )
                self.assertEqual(values.channels[9].values.data_type, ods.DataTypeEnum.DT_FLOAT)
                self.assertSequenceEqual(
                    values.channels[9].values.float_array.values, [1.100000023841858, 1.2000000476837158]
                )
                ...
                self.assertEqual(values.channels[12].values.data_type, ods.DataTypeEnum.DT_DATE)
                self.assertSequenceEqual(
                    values.channels[12].values.string_array.values, ["20170709123500", "20170709123600"]
                )
                self.assertEqual(values.channels[13].values.data_type, ods.DataTypeEnum.DT_DATE)
                self.assertSequenceEqual(
                    values.channels[13].values.string_array.values, ["20170709123500", "20170709123600"]
                )
            finally:
